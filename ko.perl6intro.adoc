...
이 문서는 Perl6 프로그래밍 언어의 개요를 간략한 개요를 제공합니다.
Perl6를 새로 시작하는 사람들에게, 시작하는 것을 목적으로 합니다.
...
Perl 6(펄6)은 고수준, 일반 타입언어이다.

Perl 6 is multi-paradigmatic. It supports Procedural, Object Oriented, and Functional programming.

Perl 6는 멀티 패러다임 언어이다. 절차지향, 객제 지향, 함수형 프로그래밍을 지원한다. 

.Perl 6 표어(Perl 6 motto):
...
=== 펄 6 코드를 실행시키기(Running Perl 6 code)
...
그리고 [Enter]버튼을 누른다. 그러면 명령 프롬에 `>`가 나타난다.
다음, 코드의 줄을 입력을 하고 [Enter]버튼을 누른다. 그러면 REPL이 해당 줄에 대한 값을 출력할 것이다.
...
Perl 6 스크립트에는`.pl6` 파일 이름 확장자를 가지는 것을 추천한다.
터미널 창에 `perl6 filename.pl6`을 입력하여 파일을 실행하십시오.
그리고 [Enter]를 치십시오. REPL과 달리 이것은 자동으로 출력되지 않습니다.
...
| 연산자(Operator) | 유형/타입(Type) | 설명(Description) | 예제(Example) | 결과(Result)
...
주석: Integer에 대한 적합한 메소드의 목록은 다음의 링크를 확인하면 됩니다. https://docs.perl6.org/type/Int
