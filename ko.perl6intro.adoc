...
NOTE: 연산자 우선순위를 포함한 연산자 목록의 상세내용을 보려면 다음의 링크를 참조하시기 바랍니다. https://docs.perl6.org/language/operators
...
NOTE: Strings에 적합한 메소드 목록은 다음의 링크를 확인 하면 됩니다. https://docs.perl6.org/type/Str
...
NOTE: 정수(Integer)에 대한 적합한 메소드의 목록은 다음의 링크를 확인하면 됩니다. https://docs.perl6.org/type/Int
...
NOTE: 유리수(Rational Number)에 대한 적합한 메소드의 목록은 다음의 링크를 확인하면 됩니다. https://docs.perl6.org/type/Rat
...
NOTE: 서브루틴과 함수에 대한 더 많은 정보는 다음을 참조하시기 바랍니다. https://docs.perl6.org/language/functions
...
* 혹은 `a`의 코드포인트와 양음 약센트 기호(´)의 코드포인트 `\x0061\x0301` 를 결합하여 사용한다
...
일반적인 상황에서는, 프로그램의 모든 태스크(task)는 순차적으로 동작합니다. +
이 활동이 많은 시간을 소비하는 것이 아니라면 문제가 되지 않습니다.
...
물론, Perl6는 병렬로 동작할 수 있는 기능을 가지고 있습니다. +
이번 장에서는 병렬처리(Parallelism)는 2가지 중의 하나를 의미한다는 것이 중요합니다.

* *Task Parallelism*: Two (or more) independent expressions running in parallel.

* *태스크 병렬처리*: 두 개 이상의 독립적인 태스크가 병렬 실행이 됩니다.

* *Data Parallelism*: A single expression iterating over a list of elements in parallel.

* *데이터 병렬처리*: 하나의 식이 요소 목록에 병렬적으로 반복합니다.
...
.위의 예제를 봅시다.
단지 한 개의 명령어 `@array.map({ is-prime $_ })`로 실행할 수 있습니다.
`is-prime` 서브루틴은 순차적으로 각각의 배열에 호출을 하고 있습니다.
`is-prime @array[0]`, `is-prime @array[1]`, `is-prime @array[2]` 등등.

.Fortunately we can call `is-prime` on multiple array elements at the same time:
.운이 좋게, 동시에 다중 배열 요소를 `is-prime`로 호출할 수 있습니다:
[source,perl6]
----
my @array = (0..50000);                         #Array population
...
위의 두가지 예제를 실행하면, 하나는 정렬이 되고 하나는 정렬이 되지 않은 것을 알 수 있습니다.
...
NOTE: 병행성과 비동기 프로그래밍에 대한 더 많은 정보에 대하여, https://docs.perl6.org/language/concurrency 를 참조하세요.
